...
Generate some random data with the function `podlove_create_example()` with ~10.000 downloads in total:

```{r}
table_list <- podlove_create_example(total_dls = 10000, seed = 12)
```

The `seed` parameter fixes the randomization to give you the same data as in this example. The result is a list of 5 named tables (`posts`, `episodes`, `mediafiles`, `useragents`, `downloads`) wrapped in a list. Now all you have to do is feed them to the cleaning function:
...
By the way, if you want get clean data directly without having to create tables, you can set the `podlove_create_example()` parameter `clean = TRUE`:
